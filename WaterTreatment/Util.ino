...
uint8_t initSD(void)
{
	...
	if(!card.card()->readCID(&cid)) {
		...
		MC.message.setMessage(pMESSAGE_SD, (char*) "Ошибка чтения информации о карте (readCID)", 0); // сформировать уведомление
		SPI_switchW5200();
		return 0;
	}
	...
	if(!card.exists(INDEX_FILE)) {
		...
		MC.message.setMessage(pMESSAGE_SD, (char*) "Файл индекса не найден на SD карте", 0); // сформировать уведомление
		SPI_switchW5200();
		return 1;
...
// Возврат 2 - если веб есть на карте (готово к использованию), 1 - флеш есть, но веба нет, 0 - диск не работaет или нет морды (использовать нельзя)
#ifdef SPI_FLASH
uint8_t initSpiDisk(boolean show)
{
	...
	if(!SerialFlash.begin(PIN_SPI_CS_FLASH)) {
		...
		if (MC.get_WebStoreOnSPIFlash()) { // проверка наличия файла INDEX_FILE, если стоит флаг загрузки из флеша в настройках
			if (!SerialFlash.exists((char*)INDEX_FILE)) { // файл не найден морды во флеше нет
				MC.set_WebStoreOnSPIFlash(false);//  сбрасываем в оперативке (но не на флеше) флаг загрузки из флеш диска
				if(show) journal.jprintf((char*) " ERROR - Can't find %s file on SPI flash!\n", INDEX_FILE);
				MC.message.setMessage(pMESSAGE_SD, (char*) "Файл индекса не найден на SPI флеш диске", 0);// сформировать уведомление
				return 1;
...
inline tmElements_t *getTime_RtcI2C()
{
	if(SemaphoreTake(xI2CSemaphore, I2C_TIME_WAIT / portTICK_PERIOD_MS) == pdFALSE) {  // Если шедулер запущен то захватываем семафор
...
int8_t save_struct(uint32_t &addr_to, uint8_t *addr_from, uint16_t size, uint16_t &crc)
{
	if(size < 128) { // Меньше 128 байт, длина 1 байт, первый бит = 0
		size <<= 1;
		if(writeEEPROM_I2C(addr_to++, (uint8_t *)&size, 1)) return set_Error(ERR_SAVE_EEPROM, (char*)__FUNCTION__);
		crc = _crc16(crc, size);
	} else { // длина 2 байта, первый бит = 1
		size = (size << 1) | 1;
		if(writeEEPROM_I2C(addr_to, (uint8_t *)&size, 2)) return set_Error(ERR_SAVE_EEPROM, (char*)__FUNCTION__);
		addr_to += 2;
	...
	if(writeEEPROM_I2C(addr_to, addr_from, size)) {
		return set_Error(ERR_SAVE_EEPROM, (char*)__FUNCTION__);
	}
	addr_to += size;
...
{
	if(writeEEPROM_I2C(addr_to, (uint8_t *)&data, 2)) return set_Error(ERR_SAVE_EEPROM, (char*)__FUNCTION__);
	addr_to += 2;
